...
int main()
{
  ...
  int radius[9] = { 5, 8, 11, 14, 13, 10, 7, 6, 9 };
  int radiusflag[9] = { 0, 1, 0, 1, 0, 1, 0, 1, 0 };
  ...
  for(i = 0; i < 9; i++)
     {
	 circle(axis[i][0], axis[i][1], radius[i]);
     }
  ...
  while(!kbhit())
    {
      ...
      for(i = 0; i < 9; i++)
	 {
	  if( flag[i][1] == 0 )
	    {
	     if( axis[i][1] >= (getmaxy() - radius[i] - 5) )
	       {
		flag[i][1] = 1;
	       }
	     axis[i][1]+=1;
	  ...
	  if( flag[i][0] == 0 )
	    {
	     if( axis[i][0] >= (getmaxx() - radius[i] - 5) )
	       {
		flag[i][0] = 1;
	       }
	     axis[i][0]+=1;
